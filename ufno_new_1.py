...
class U_net(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, dropout_rate=0.1):
        ...
        self.conv3 = self.conv(output_channels, output_channels*2, kernel_size=kernel_size, stride=2, dropout_rate=dropout_rate)
        self.conv3_1 = self.conv(output_channels*2, output_channels*2, kernel_size=kernel_size, stride=1, dropout_rate=dropout_rate)
        
        self.deconv2 = self.deconv(output_channels*2, output_channels)
    ...
    def forward(self, x):
        ...
        out_deconv2 = self.deconv2(out_conv3) + out_conv2  # Residual connection
        concat2 = torch.cat((out_conv2, out_deconv2), 1)
        out_deconv1 = self.deconv1(concat2) + out_conv1  # Residual connection
...
class SimpleBlock3d(nn.Module):
    def __init__(self, modes1, modes2, modes3, width):
        ...
        self.modes3 = modes3
        self.width = width
        self.fc0 = nn.Linear(1, self.width)  # Updated for 2 input channels
        ...
        self.unet3 = U_net(self.width, self.width, 3, 0.1)
        self.unet4 = U_net(self.width, self.width, 3, 0.1)
        self.unet5 = U_net(self.width, self.width, 3, 0.1)
